...
/// What drops down from the menu bar: every account's two usage windows, and
/// the few controls worth having without opening the window.
struct MenuBarContent: View {
        ...
        VStack(alignment: .leading, spacing: 0) {
            if usage.entries.isEmpty {
                ...
                ForEach(usage.entries) { entry in
                    ...
                    if entry.id != usage.entries.last?.id { Divider().padding(.leading, 14) }
                }
            }
    ...
    private func startSession(_ entry: UsageMonitor.Entry) {
        ...
        if !SessionStarter.hasAccessibility {
            SessionStarter.requestAccessibility()
        }
        ...
        DispatchQueue.global(qos: .userInitiated).async {
            let failure = SessionStarter.start(profile: entry.profile, bundle: bundle)
